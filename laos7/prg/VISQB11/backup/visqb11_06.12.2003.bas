...
DIM SHARED ctlX, ctlY, ctlX2, ctlY2, ctlCaption, ctlSet
...
SUB Dialog (Text AS STRING)
  ...
  x2 = 5 + l + w
  y2 = t + h
  IF SilentScreenMode OR SaveScreenMode THEN
...
vqb99:
  ...
  END IF
  DefaultCtrl = ctlSet
  ShortKeys = "f5,#63,##,#64,#65,#66,#67,#68,insert,t,l,r,b,delete,@15,@19,@14,@12,@17"
  GetInp ""
  Markieren 0, 0, 0, 0
  SELECT CASE keyb
  CASE "f5"
  ...
  CASE "Setzen"
    ...
    CASE zIconList
      ...
      Controls(LastActivated + 1).x = x2 + 1
      Controls(LastActivated + 1).y = y
      Controls(LastActivated + 1).x2 = x2 + 14
      Controls(LastActivated + 1).y2 = y2
    CASE zListBox
  ...
  CASE "esc"
    ...
    END IF
  CASE "Datei"
    a$ = MenuInt("Neu|Ctrl+N,-,ôffnen...|Ctrl+L,Speichern|Ctrl+S,Speichern unter...,-,Auf QBasic-Programm speichern,-,Beenden")
    SELECT CASE a$
...
vqbsaveas:
  ...
  CASE "Objekte"
    MenuObjekte
  CASE "Bearbeiten"
    a$ = MenuInt$("Lîschen,-,Eigenschaften")
    SELECT CASE a$
    CASE "Lîschen"
vqbdelete:
      IF LastActivated < 0 THEN
...
SUB MenuExtras
  a$ = MenuInt$("MenÅ-Editor,Als Screenshot speichern,Optionen")
  SELECT CASE a$
  CASE "MenÅ-Editor"
  CASE "Optionen"
    Options
...
SUB MenuObjekte
  a$ = MenuInt("Text|t,Fliesstext,-,Frame,-,Button,Editbox,ListBox,CheckBox,RadioButton,IconList")
  ToInsert = 0
  SELECT CASE a$
  '*1
  CASE "Text"
...
SaveScreen
  ...
  MouseClicker "options", "color", 340, 150, 370, 165, "enter", 0
...
SUB RaiseGotFocus (iOldCtrl, iCtrl, name$)
  IF RTRIM$(Controls(ActiveCtrl).Group) = "user" THEN
    ...
    MID$(Controls(ctlCaption).tag, 1, 2) = MKI$(40)
    DrawControls "prop"
    LastActivated = iCtrl
  END IF
END SUB
...
IF RTRIM$(Controls(iCtrl).Group) = "user" AND iButton = 1 THEN
'Standard-Control
  x = Controls(iCtrl).x
  y = Controls(iCtrl).y
  ...
  IF Mouse.Oldx >= x AND Mouse.Oldy >= y THEN
    IF Mouse.Oldx <= x2 AND Mouse.Oldy <= y2 THEN
      ...
      CASE zIconList, zListBox
        ...
        'Scrollbar aktualisieren:
        Controls(iCtrl + 1).x = x2 + 1
        Controls(iCtrl + 1).y = y
        Controls(iCtrl + 1).x2 = x2 + 14
        Controls(iCtrl + 1).y2 = y2
      CASE ELSE
        'normal verschieben!
        Controls(iCtrl).x2 = x2 + px
        Controls(iCtrl).y2 = y2 + py
...
ELSEIF name$ = "pane" THEN
  ...
  ELSEIF LastActivated > 0 THEN
    ...
    CASE zListBox, zIconList
      ...
      IF CVI(MID$(Controls(LastActivated).tag, 3, 2)) > 0 THEN
...
DIM oFile AS ObjectType
  ...
  FOR i = 1 TO UBOUND(Controls)
    ...
    CASE zIconList
      ...
      'LINE (Controls(i).x, Controls(i).y)-(Controls(i).x2, Controls(i).y2), 0, B
      zDrawCtrl i, 0, -1
...
SUB SetActive (oFile AS ObjectType)
  ...
  ELSE
    EchoXY 270, 330, "Caption:"
  END IF
  MID$(Controls(ctlCaption).tag, 1, 2) = MKI$(160)
  GetString "caption", a$
  DrawControls "prop"
  Markieren x, y, x2, y2
END SUB
